...
impl MidiNote { pub fn validate(&self) -> bool { self.length_ticks > 0 && self.pitch < 128 && self.velocity < 128 } }
...
impl MidiFilter { pub fn validate(&self) -> bool { self.min_pitch <= self.max_pitch && self.max_pitch < 128 && self.min_vel <= self.max_vel && self.max_vel < 128 } }
...
impl MidiOrchestrator {
    pub fn save_preset(&mut self, preset: MidiTransformPreset) -> bool {
        if preset.name.trim().is_empty() || preset.name.len() > 128 || preset.name.contains('\0') || !preset.velocity_scale.is_finite() || !(0.0..=16.0).contains(&preset.velocity_scale) { return false; }
        let normalized_name = preset.name.trim().to_owned();
        if let Some(existing) = self.presets.iter_mut().find(|p| p.name.eq_ignore_ascii_case(&normalized_name)) { *existing = MidiTransformPreset { name: normalized_name, ..preset }; } else if self.presets.len() < 4096 { self.presets.push(MidiTransformPreset { name: normalized_name, ..preset }); } else { return false; }
        true
    }

    pub fn apply_preset(&self, name: &str, notes: &mut [MidiNote], filter: &MidiFilter) -> bool {
        let Some(preset) = self.presets.iter().find(|p| p.name.eq_ignore_ascii_case(name.trim())) else { return false; };
        if !filter.validate() || notes.iter().any(|note| !note.validate()) || preset.pitch_offset.abs() > 127 { return false; }
        for note in notes.iter_mut().filter(|n| n.pitch >= filter.min_pitch && n.pitch <= filter.max_pitch && n.velocity >= filter.min_vel && n.velocity <= filter.max_vel) {
    ...
    pub fn preset_names(&self) -> Vec<String> {
        let mut names: Vec<_> = self.presets.iter().map(|preset| preset.name.clone()).collect();
    ...
    pub fn remove_preset(&mut self, name: &str) -> bool {
        ...
        self.presets.retain(|preset| !preset.name.eq_ignore_ascii_case(name.trim()));
        ...
        self.presets.len() <= 4096
            && self.presets.iter().enumerate().all(|(i, preset)| {
                !preset.name.trim().is_empty()
                    ...
                    && self.presets[..i].iter().all(|previous| !previous.name.eq_ignore_ascii_case(&preset.name))
            })
    }
}

/// Applies non-destructive note-length and velocity shaping to a selected range.
pub fn transform_timing(notes: &mut [MidiNote], min_pitch: u8, max_pitch: u8, length_scale: f32, velocity_delta: i16) -> bool {
    if min_pitch > max_pitch || !length_scale.is_finite() || length_scale < 0.0 || length_scale > 64.0 { return false; }
    for note in notes.iter_mut().filter(|n| n.pitch >= min_pitch && n.pitch <= max_pitch) {
        note.length_ticks = ((note.length_ticks as f32 * length_scale).round()).clamp(1.0, u32::MAX as f32) as u32;
...
pub fn repeat_notes(notes: &[MidiNote], repeats: u32, spacing_ticks: u64) -> Option<Vec<MidiNote>> {
    if repeats == 0 || repeats > 1024 || notes.iter().any(|note| !note.validate()) { return None; }
    let mut out = Vec::with_capacity(notes.len().saturating_mul(repeats as usize));
    for repeat in 0..repeats { let offset = spacing_ticks.checked_mul(repeat as u64)?; for note in notes { let mut copy = note.clone(); copy.start_tick = copy.start_tick.checked_add(offset)?; out.push(copy); } }
    Some(out)
}
...
pub fn quantize_note_lengths(notes: &mut [MidiNote], grid_ticks: u32) -> bool {
    if grid_ticks == 0 || notes.iter().any(|note| !note.validate()) { return false; }
    for note in notes {
        let rounded = ((note.length_ticks as u64 + grid_ticks as u64 / 2) / grid_ticks as u64).max(1) * grid_ticks as u64;
...
pub fn apply_note_probability(notes: &[MidiNote], probability: f32, seed: u64) -> Option<Vec<MidiNote>> {
    if !probability.is_finite() || !(0.0..=1.0).contains(&probability) || notes.iter().any(|note| !note.validate()) { return None; }
    let threshold = (probability * u64::MAX as f32) as u64;
    let mut state = seed;
    let mut out = Vec::with_capacity(notes.len());
    for note in notes {
        state = state.wrapping_mul(6364136223846793005).wrapping_add(1);
        if state <= threshold { out.push(note.clone()); }
    }
    Some(out)
}
...
mod repeat_tests {
    ...
    fn repeats_with_offsets() {
        let n = MidiNote { start_tick: 4, length_ticks: 8, pitch: 60, velocity: 100 };
        let out = repeat_notes(&[n], 3, 16).unwrap();
        assert_eq!(out.iter().map(|n| n.start_tick).collect::<Vec<_>>(), vec![4, 20, 36]);
    }

    #[test]
    fn rejects_midi_filter_values_outside_seven_bit_range() {
        let invalid_pitch = MidiFilter { min_pitch: 0, max_pitch: 200, min_vel: 1, max_vel: 127 };
        let invalid_velocity = MidiFilter { min_pitch: 0, max_pitch: 127, min_vel: 1, max_vel: 200 };
    ...
    fn named_transform_presets_apply_and_list_deterministically() {
        ...
        assert!(orchestrator.save_preset(MidiTransformPreset { name: "Transpose".into(), pitch_offset: 12, velocity_scale: 0.5 }));
        assert_eq!(orchestrator.preset_names(), vec!["Transpose"]);
        let mut notes = vec![MidiNote { start_tick: 0, length_ticks: 10, pitch: 60, velocity: 100 }];
        let filter = MidiFilter { min_pitch: 0, max_pitch: 127, min_vel: 0, max_vel: 127 };
...
impl MidiOrchestrator {
    ...
    pub fn transform_notes(
        ...
        if !filter.validate() || !vel_scale.is_finite() { return; }
    ...
    pub fn apply_scale_quantize(&self, notes: &mut [MidiNote], root: u8, scale: &[i32]) {
        if scale.is_empty() || root > 11 || scale.iter().any(|degree| !(-24..=24).contains(degree)) || notes.iter().any(|note| !note.validate()) { return; }
    ...
    pub fn audit_midi(&self) -> bool {
        self.audit_presets()
            && self.presets.iter().all(|preset| preset.pitch_offset.abs() <= 127)
    }
}

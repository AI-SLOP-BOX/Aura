...
mod tests {
    ...
    fn routes_and_rejects_cycles() {
        let mut r=RoutingOrchestrator::new(); assert!(r.connect(1,2)); assert!(r.connect(1,3));
        assert_eq!(r.destinations(1), &[2,3]); assert!(!r.connect(2,1)); assert!(r.disconnect(1,2));
        assert_eq!(r.destinations(1), &[3]); assert!(r.audit_bus_router());
    }
...
impl RoutingOrchestrator {
    ...
    pub fn connect(&mut self, source: u32, destination: u32) -> bool {
        ...
        self.rebuild_graph();
        if self.cached_levels.iter().map(|level| level.nodes.len()).sum::<usize>()
            >= self.all_nodes.len()
        {
            true
        } else {
    ...
    pub fn disconnect(&mut self, source: u32, destination: u32) -> bool { let Some(edges)=self.adj.get_mut(source as usize) else { return false; }; let before=edges.len(); edges.retain(|id| *id != destination); let removed=before != edges.len(); if removed { self.route_gains.remove(&(source, destination)); self.rebuild_graph(); } removed }
    pub fn remove_node(&mut self, node: u32) -> bool {
        if !self.all_nodes.remove(&node) { return false; }
        if let Some(edges) = self.adj.get_mut(node as usize) { edges.clear(); }
        for edges in &mut self.adj { edges.retain(|destination| *destination != node); }
        self.route_gains.retain(|(source, destination), _| *source != node && *destination != node);
        self.rebuild_graph();
        true
    }
    pub fn destinations(&self, source: u32) -> &[u32] { self.adj.get(source as usize).map(Vec::as_slice).unwrap_or(&[]) }
    ...
    pub fn set_route_gain(&mut self, source: u32, destination: u32, gain: f32) -> bool {
        if !gain.is_finite() || !(-8.0..=8.0).contains(&gain) || !self.destinations(source).contains(&destination) { return false; }
        self.route_gains.insert((source, destination), gain);
        true
    }

    pub fn route_gain(&self, source: u32, destination: u32) -> Option<f32> { self.route_gains.get(&(source, destination)).copied() }
    ...
    pub fn mix_direct_routes(&self, source: u32, input: &[f32], outputs: &mut std::collections::HashMap<u32, Vec<f32>>) -> bool {
        if input.is_empty() || input.len() > 16_000_000 || input.iter().any(|sample| !sample.is_finite()) { return false; }
        ...
        if destinations.iter().any(|destination| outputs.get(destination).is_some_and(|output| output.len() != input.len())) { return false; }
        for &destination in destinations {
            let gain = self.route_gain(source, destination).unwrap_or(1.0);
            let output = outputs.entry(destination).or_insert_with(|| vec![0.0; input.len()]);
            for (slot, sample) in output.iter_mut().zip(input) { *slot += *sample * gain; }
        }
        true
    }
        ...
        self.cached_levels.iter().all(|level| {
            level.nodes.windows(2).all(|pair| pair[0] != pair[1])
                && level
                    .nodes
                    .iter()
                    .all(|id| (*id == 0 || self.all_nodes.is_empty() || self.all_nodes.contains(id)) && (*id == 0 || seen.insert(*id)))
        }) && seen.len() == self.all_nodes.len()
            && self.route_gains.iter().all(|((source, destination), gain)| self.destinations(*source).contains(destination) && gain.is_finite() && (-8.0..=8.0).contains(gain))
    }
}

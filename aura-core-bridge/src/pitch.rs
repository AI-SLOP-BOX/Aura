...
fn default_timing_ratio() -> f32 { 1.0 }

impl PitchBlock {
    pub fn validate(&self) -> bool {
        self.start_sample < self.end_sample
            && self.target_note.is_finite() && (0.0..=127.0).contains(&self.target_note)
            && self.vibrato_amount.is_finite() && self.drift_amount.is_finite()
            && self.vibrato_amount.abs() <= 1.0 && self.drift_amount.abs() <= 1.0
            && self.micro_pitch_cents.is_finite() && self.micro_pitch_cents.abs() <= 1200.0
            && self.pitch_curve_cents.len() <= 4096
            && self.pitch_curve_cents.iter().all(|value| value.is_finite() && value.abs() <= 1200.0)
            && self.formant_shift_semitones.is_finite() && self.formant_shift_semitones.abs() <= 24.0
            && self.timing_ratio.is_finite() && (0.25..=4.0).contains(&self.timing_ratio)
    }
}
...
impl PitchOrchestrator {
    ...
    pub fn upsert_block(&mut self, block: PitchBlock) -> bool {
        ...
        let replacement = self.blocks.iter().position(|b| b.start_sample == block.start_sample && b.end_sample == block.end_sample);
        if self.blocks.iter().enumerate().any(|(i, other)| Some(i) != replacement && block.start_sample < other.end_sample && other.start_sample < block.end_sample) { return false; }
        if let Some(existing) = self.blocks.iter_mut().find(|b| b.start_sample == block.start_sample && b.end_sample == block.end_sample) { *existing = block; } else { self.blocks.push(block); }
        self.blocks.sort_by_key(|b| b.start_sample); true
    }

    pub fn remove_block(&mut self, start_sample: u64, end_sample: u64) -> bool {
        let before = self.blocks.len(); self.blocks.retain(|b| !(b.start_sample == start_sample && b.end_sample == end_sample)); before != self.blocks.len()
    }
    ...
    pub fn import_detected_segments(&mut self, detected: &[crate::pitch_detector::DetectedPitchSegment]) -> bool {
        if detected.len() > 1_000_000 || detected.iter().any(|segment| segment.start_sample >= segment.end_sample || segment.midi_note > 127 || !segment.frequency_hz.is_finite() || segment.frequency_hz <= 0.0 || !segment.confidence.is_finite() || !(0.0..=1.0).contains(&segment.confidence)) { return false; }
        let mut blocks = Vec::with_capacity(detected.len());
        for segment in detected {
            blocks.push(PitchBlock { start_sample: segment.start_sample, end_sample: segment.end_sample, target_note: segment.midi_note as f32, vibrato_amount: 0.0, drift_amount: 0.0, micro_pitch_cents: 0.0, pitch_curve_cents: Vec::new(), formant_shift_semitones: 0.0, timing_ratio: 1.0 });
        }
        blocks.sort_by_key(|block| block.start_sample);
        if blocks.windows(2).any(|pair| pair[0].end_sample > pair[1].start_sample) { return false; }
        self.blocks = blocks;
        true
    }
    ...
    pub fn get_shift_ratio(&self, now: u64, detected_freq: f32) -> f32 {
        ...
        if let Some(block) = self.find_block(now) {
            ...
            let target_note = self.target_note_at(now).unwrap_or(block.target_note).clamp(0.0, 127.0);
            let target_freq = 440.0 * 2.0f32.powf((target_note - 69.0) / 12.0)
    ...
    pub fn get_formant_ratio(&self, now: u64) -> f32 {
        self.find_block(now)
            .map(|block| 2.0f32.powf(block.formant_shift_semitones / 12.0).clamp(0.25, 4.0))
            .unwrap_or(1.0)
    }

    pub fn get_timing_ratio(&self, now: u64) -> f32 {
        self.find_block(now).map(|block| block.timing_ratio).unwrap_or(1.0)
    }
    ...
    pub fn set_pitch_curve(&mut self, start_sample: u64, end_sample: u64, curve_cents: Vec<f32>) -> bool {
        if curve_cents.len() > 4096 || curve_cents.iter().any(|value| !value.is_finite() || value.abs() > 1200.0) { return false; }
        let Some(block) = self.blocks.iter_mut().find(|block| block.start_sample == start_sample && block.end_sample == end_sample) else { return false; };
    ...
    pub fn target_note_at(&self, now: u64) -> Option<f32> {
        ...
        let vibrato = block.vibrato_amount.clamp(-1.0, 1.0) * (phase * std::f32::consts::TAU * 5.0).sin();
    ...
    fn find_block(&self, now: u64) -> Option<&PitchBlock> {
        self.blocks.iter().find(|b| now >= b.start_sample && now < b.end_sample)
    }
    ...
    pub fn audit_pitch(&self) -> bool {
        self.blocks.iter().all(PitchBlock::validate)
            && self.blocks.windows(2).all(|pair| pair[0].end_sample <= pair[1].start_sample)
    }
}
...
mod tests {
    ...
    fn editor_segments_are_sorted_and_non_overlapping() {
        let mut pitch = PitchOrchestrator::new();
        let block = |start, end| PitchBlock { start_sample: start, end_sample: end, target_note: 69.0, vibrato_amount: 0.0, drift_amount: 0.0, micro_pitch_cents: 0.0, pitch_curve_cents: Vec::new(), formant_shift_semitones: 0.0, timing_ratio: 1.0 };
    ...
    fn segment_exposes_fine_pitch_formant_and_timing_controls() {
        let mut pitch = PitchOrchestrator::new();
        assert!(pitch.upsert_block(PitchBlock { start_sample: 0, end_sample: 100, target_note: 69.0, vibrato_amount: 0.0, drift_amount: 0.0, micro_pitch_cents: 100.0, pitch_curve_cents: Vec::new(), formant_shift_semitones: 12.0, timing_ratio: 1.5 }));
    ...
    fn detector_segments_import_as_editable_blocks_atomically() {
        let mut pitch = PitchOrchestrator::new();
        let detected = vec![crate::pitch_detector::DetectedPitchSegment { start_sample: 100, end_sample: 200, midi_note: 60, frequency_hz: 261.6, confidence: 0.9 }];
        assert!(pitch.import_detected_segments(&detected));
        assert_eq!(pitch.blocks[0].target_note, 60.0);
        let invalid = vec![crate::pitch_detector::DetectedPitchSegment { start_sample: 150, end_sample: 250, midi_note: 61, frequency_hz: 277.0, confidence: 0.9 }];
    ...
    fn target_note_resolution_applies_bounded_expression_curve() {
        let mut pitch = PitchOrchestrator::new();
        assert!(pitch.upsert_block(PitchBlock { start_sample: 0, end_sample: 100, target_note: 60.0, vibrato_amount: 0.5, drift_amount: 0.25, micro_pitch_cents: 0.0, pitch_curve_cents: Vec::new(), formant_shift_semitones: 0.0, timing_ratio: 1.0 }));
    ...
    fn editable_pitch_curve_is_interpolated_in_cents() {
        let mut pitch = PitchOrchestrator::new();
        assert!(pitch.upsert_block(PitchBlock { start_sample: 0, end_sample: 100, target_note: 60.0, vibrato_amount: 0.0, drift_amount: 0.0, micro_pitch_cents: 0.0, pitch_curve_cents: Vec::new(), formant_shift_semitones: 0.0, timing_ratio: 1.0 }));
